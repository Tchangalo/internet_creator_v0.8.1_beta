...
ansible-playbook -i inventories/inventory${provider}.yaml vyos_upgrade_fast.yml -e "vyos_version=$(ls -t ${HOME}/streams/ansible/vyos-files/ | head -n 1 | sed -e 's/^vyos-//' | sed -e 's/-amd.*$//')" "$vyos_ansible_limit"
...
for r in $(seq $fr $lr); do
    sudo qm shutdown ${provider}0${provider}00$r
done
